...
const CustomNode: FC<NodeProps<Course>> = ({ data, selected }) => {
  return (
    <>
      ...
      <Card
        className={`w-64 border-2 transition-all duration-200 hover:shadow-lg hover:border-primary/80
                    ${selected ? 'border-primary shadow-xl scale-105' : 'border-transparent shadow-md'}
      ...
      >
        <CardHeader className="flex flex-row items-center gap-4 p-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground dark:text-gray-300">{data.title}</p>
          </div>
        </CardHeader>
      </Card>
