    ...
    <div className="flex h-screen w-screen overflow-hidden bg-background text-foreground">
        ...
        <div className="bg-background/80 backdrop-blur-sm p-2 px-4 rounded-lg pointer-events-auto shadow-sm border flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {Array.from(new Set((diagramCourses || []).map((c) => c.subject)))
                .filter(Boolean)
                .sort()
                .map((s) => (
            ...
            <Button
              ...
              "
            >
              {plannerOpen ? "Close Planner" : "Course Planner"}
            </Button>
          </div>
        </div>
      </header>
      ...
      >
        ...
        <Button
          ...
          className={cn(
            "h-10 rounded-full shadow-lg bg-background/80 backdrop-blur-sm border flex items-center gap-3 transition-all",
            isSidebarVisible ? "px-3" : "px-4"
          )}
          aria-label={
            isSidebarVisible ? "Hide sidebar" : "Show details sidebar"
          }
        >
          {isSidebarVisible ? (
            <>
      ...
      {isMobile ? (
        <Sheet
          ...
          onOpenChange={(open) => setSidebarMode(open ? sidebarMode : "closed")}
        >
